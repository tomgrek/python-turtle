...
class Policy(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(5, 36, kernel_size=2, stride=1, padding=2)
        ...
        self.action_head = nn.Linear(30, 3)
    ...
    def forward(self, x, last_action):
        ...
        action_scores = torch.tanh(self.action_head(x))
    ...
    def act(self, state, last_action, episode):
        probs, state_value = self.forward(state, last_action)
...
for episode in range(0, 500000):
    ...
    last_action = 0.
    while not done:
        action = model.act(torch.cat((state, env.tortoise.prev_frames[0], old_state, older_state, oldest_state)), last_action, episode)
        last_action = action
